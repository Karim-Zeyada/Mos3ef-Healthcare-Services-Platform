...
import { Pen, Trash2, Plus, Star, ClipboardList, BarChart3 } from "lucide-react";
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `http://localhost:5000/api/Hospital/GetDashboardStats/${user.hospitalId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const stats = res.data.data;
        setDashboardStats({
          servicesCount: stats.servicesCount || stats.item1 || 0,
          reviewsCount: stats.reviewsCount || stats.item2 || 0,
          avgRating: stats.avgRating || stats.item3 || 0,
        });
      } catch (err) {
  ...
  return (
    <section className="flex flex-col flex-1 min-w-0">
      ...
      <div className="flex flex-col flex-1 w-full max-w-[863.2px] mx-auto gap-8 px-2 py-3 md:p-8 overflow-x-hidden">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-fade-in [--animation-delay:200ms]">
          <Card className="bg-gradient-to-br from-Blue to-Blue-900 text-white rounded-2xl border-0">
            <CardContent className="flex items-center justify-between p-5">
              <ClipboardList className="w-10 h-10 opacity-70" />
              <div className="text-right">
                <p className="font-Cairo text-3xl font-bold">{dashboardStats.servicesCount}</p>
                <p className="font-Cairo text-sm opacity-80">إجمالي الخدمات</p>
              </div>
            </CardContent>
          </Card>
          <Card className="bg-gradient-to-br from-Green to-Green-600 text-white rounded-2xl border-0">
            <CardContent className="flex items-center justify-between p-5">
              <BarChart3 className="w-10 h-10 opacity-70" />
              <div className="text-right">
                <p className="font-Cairo text-3xl font-bold">{dashboardStats.reviewsCount}</p>
                <p className="font-Cairo text-sm opacity-80">إجمالي التقييمات</p>
              </div>
            </CardContent>
          </Card>
          <Card className="bg-gradient-to-br from-yellow-400 to-yellow-600 text-white rounded-2xl border-0">
            <CardContent className="flex items-center justify-between p-5">
              <Star className="w-10 h-10 opacity-70" />
              <div className="text-right">
                <p className="font-Cairo text-3xl font-bold">{dashboardStats.avgRating.toFixed(1)}</p>
                <p className="font-Cairo text-sm opacity-80">متوسط التقييم</p>
              </div>
            </CardContent>
          </Card>
        </div>
